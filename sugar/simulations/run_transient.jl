...
function simulate_transient(network::RxnNetwork)
    ...
    teval = (10 .^(range(2,stop=log10(tspan[2]),length=1000))...,)
    ...
    for s in species_ids
        ...
        init_conc = get_initial_conc(network.species,[(Species(id=s), 0.2),(Species(id="Catalyst"), 0.004)])

        run = simulate_timecourse(network, init_conc, tspan, teval).u

        # get the maximum conentration of each species along the simulated simulated timecourse
