...
function simulate_asymptote(network::RxnNetwork)
    """simulate the asymptote of a network, i.e. the start independent steady state
    aproximate as when starting at opposite nodes (ie glucose and idose) the final concentrations are the same

    Args:
        network (RxnNetwork): network to simulate

    Returns:
        Vector{Float64}: final concentrations
    """

    # initial timespan -- 50_000 is arbitrary and enough for most simulations
    tspan = (0.0, 50_000.0)
...
for (correlated_connection_set, id) in zip(correlated_connections, cor_ids)
    for rate_std in stdev_params
        ...
        base_rate_sigma = base_rate_sigma_kcal / (1.9872036e-3 * 298.15) # divide by R*T
        ...
        for sel_std in stdev_params
            ...
            selectivity_sigma = selectivity_sigma_kcal / (1.9872036e-3 * 298.15) # divide by R*T
            ...
            for cor_type in correlation_types
                for bias_type in bias_types
                    ...
                    # if there is bias then adjust means
                    if bias_type != "none"
                    ...
                    for edge_cor in edge_cors
                        ...
                        try # catch errors for correlation matrix issues gracefully
                            ...
                            Threads.@threads for i in 1:number_runs
                                ...
                                # get a random sample from the distribution + save it
                                x = rand(d)
                                setups[i] = x

                                # rates are first half of x, selectivities are second half
